...
def rewrite_story_length_guard(
    *,
    ...
    if not raw_story:
        return ""
    rewrite_timeout = max(120, int(ollama_timeout_sec or 120)) if ollama_timeout_sec is not None else 120
    compress_timeout = max(90, int(ollama_timeout_sec or 90)) if ollama_timeout_sec is not None else 90
    rewrite_instruction = (
        "REWRITE-AUFTRAG:\n"
